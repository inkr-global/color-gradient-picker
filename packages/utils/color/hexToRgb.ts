...
export default function hexToRgb(colorHex: string): Rgb {
  ...
  if (sanitizedHex.length !== 6 && sanitizedHex.length !== 3) {
    throw new Error(
      `Color should be either 3 or 6 characters in length - received a length of ${sanitizedHex.length}`,
    );
  }

  if (sanitizedHex.length === 3) {
    sanitizedHex = `${sanitizedHex[0].repeat(2)}${sanitizedHex[1].repeat(
      2,
    )}${sanitizedHex[2].repeat(2)}`;
  }
